...
# s_all = np.zeros(s.shape) ; print(s_all.shape)
# for i in range(s.shape[0]):
#     for j in range(s.shape[1]):
#         if i in regions or j in regions: #OR
#             s_all[i, j] = s[i, j] * scale_all
#         else:
#             s_all[i, j] = s[i, j]
...
# for i in range(s.shape[0]):
#     for j in range(s.shape[1]):
#         if i in regions and j in regions: #AND
#             s_few[i, j] = s[i, j]
#         else:
#             s_few[i, j] = s[i, j] * scale_few
...
pos_0 = axes[0,0].imshow(s_all, interpolation='nearest', vmin=-1, vmax=1); axes[0,0].set_title("s_all)")
pos_1 =axes[0,1].imshow(s_few, interpolation='nearest', vmin=-1, vmax=1); axes[0,1].set_title("s_few)")
